unit DW.Biometric.Android.Legacy;
...
uses
  // RTL
  System.Classes, System.Permissions, System.IOUtils,
  // Android
...
begin
  inherited;
  LParts := JStringToString(TAndroidHelper.Context.getPackageName).Split(['.']);
  FKeyName := LParts[Length(LParts) - 1];
...
begin
  ...
  if HasFingerprintPermission then
  begin
    if FFingerprintManager.hasEnrolledFingerprints then
