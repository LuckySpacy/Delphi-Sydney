...
uses
  ...
  Vcl.StdCtrls, System.Types, Winapi.Messages, tbButton.PropPos, System.UITypes, Vcl.comStrs,
  Vcl.Consts, Vcl.ToolWin, Vcl.ListActns, Vcl.StdActns, Vcl.Forms, Contnrs,
  tbButton.PropNotification, tbButton.PropTextAndImageCenter, tbButton.ButtonPosBerechnung;
...
type
  TNumGlyphs = 1..4;
  TtbButtonState = (nfBtnOver, nfBtnDown, nfBtnNone);
  ...
  private
    ...
    fImagePos: TtbButtonPropPos;
    FTextAlignment: TImageAlignment;
    fTextPos: TtbButtonPropPos;
    fFlat: Boolean;
    ...
    fPropNotification: TtbButtonPropNotification;
    fShowCaption: Boolean;
    fTextAlignIgnoreImage: Boolean;
    fPosBerechnung: TtbButtonButtonPosBerechnung;
    fTextAndImageCenter: TtbButtonPropTextAndImageCenter;
  ...
  published
    ...
    property ImagePos: TtbButtonPropPos read fImagePos write fImagePos;
    property TextPos: TtbButtonPropPos read fTextPos write fTextPos;
    ...
    property NotificationText: TtbButtonPropNotification read fPropNotification write fPropNotification;
    property TextAlignIgnoreImage: Boolean read fTextAlignIgnoreImage write setTextAlignIngoreImage default false;
    property TextAndImageCenter: TtbButtonPropTextAndImageCenter read fTextAndImageCenter write fTextAndImageCenter;
    property Hint;
    property ShowHint;
...
procedure Register;
begin
  RegisterComponents('Bachmann', [TtbButton]);
end;
...
begin
  ...
  fImagePos := TtbButtonPropPos.Create;
  fImagePos.Left := 8;
  fImagePos.Right := 8;
  fImagePos.OnChanged := ImagePosChanged;

  fPropNotification := TtbButtonPropNotification.Create;
  ...
  fTextPos := TtbButtonPropPos.Create;
  fTextPos.OnChanged := TextPosChanged;

  fTextAndImageCenter := TtbButtonPropTextAndImageCenter.Create;
  fTextAndImageCenter.OnChanged := TextAndImageCenterChanged;

  fPosBerechnung := TtbButtonButtonPosBerechnung.Create;
...
procedure TtbButton.DestroyWindowHandle;
begin
  UnRegistHotKey;
  inherited;
end;

procedure TtbButton.WMNCDestroy(var Message: TWMNCDestroy);
begin
  UnRegistHotKey;
  inherited;
end;

procedure TtbButton.UnRegistHotKey;
begin
  exit;
  if fHotKeyId > 0 then
  begin
...
procedure TtbButton.DrawBitmapFromImageList(aImgList: TCustomImageList; aImageIndex: Integer);
var
  rDest: TRect;
  rSource: TRect;
begin
...
procedure TtbButton.setCaption1(const Value: string);
begin
  fCaption1 := Value;
...
procedure TtbButton.RegistHotKey;
begin
...
procedure TtbButton.setCaption2(const Value: string);
begin
  fCaption2 := Value;
...
procedure TtbButton.setColor(const Value: TColor);
begin
  fColor := Value;
  Invalidate;
end;

procedure TtbButton.setColorDown(const Value: TColor);
begin
  fColorDown := Value;
end;

procedure TtbButton.setColorSelect(const Value: TColor);
begin
  fColorSelect := Value;
end;

procedure TtbButton.SetDisabledImageIndex(const Value: Integer);
begin
  FDisabledImageIndex := Value;
  Invalidate;
end;

procedure TtbButton.SetDisabledImages(const Value: TCustomImageList);
begin
  fDisabledImages := Value;
  Invalidate;
end;

procedure TtbButton.SetEnabled(Value: Boolean);
begin
  inherited;
  Invalidate;
end;

procedure TtbButton.setFlat(const Value: Boolean);
begin
  fFlat := Value;
  Invalidate;
end;

procedure TtbButton.setFont(const Value: TFont);
begin
  fFont.Assign(Value);
  ParentFont := false;
end;

procedure TtbButton.getPicturePos(var p: TPoint);
var
  bmp_Height: Integer;
  bmp_Width: Integer;
  h: Integer;
...
procedure TtbButton.SetTextPosByUseTextAndImageCenter;
var
  i1: Integer;
  TextPoint: TTextPoint;
begin
...
procedure TtbButton.setTextPos;
var
  txt_Width: Integer;
  p_bmp: TPoint;
  h: Integer;
...
procedure TtbButton.SizeMove(var msg: TWMSize);
begin
  fPropNotification.Width := Width;
  fPropNotification.Height := Height;
end;
...
procedure TtbButton.ImagePosChanged(Sender: TObject);
begin
  Paint;
end;
...
procedure TtbButton.NotificationTextChanged(Sender: TObject);
begin
  Invalidate;
end;

procedure TtbButton.SetImageAlignment(const Value: TImageAlignment);
begin
...
procedure TtbButton.SetImages(const Value: TCustomImageList);
begin
...
procedure TtbButton.SetName(const NewName: TComponentName);
begin
  inherited;
...
procedure TtbButton.setNumGlyps(const Value: TNumGlyphs);
begin
  fNumGlyphs := Value;
  Invalidate;
end;

procedure TtbButton.setRoundRect(const Value: Integer);
begin
  fRoundRect := Value;
  Invalidate;
end;

procedure TtbButton.setShowAccelChar(const Value: Boolean);
begin
...
procedure TtbButton.setShowCaption(const Value: Boolean);
begin
  fShowCaption := Value;
  Invalidate;
end;

procedure TtbButton.setTextAlignIngoreImage(const Value: Boolean);
begin
  fTextAlignIgnoreImage := Value;
  Invalidate;
end;

procedure TtbButton.SetTextAlignment(const Value: TImageAlignment);
begin
...
procedure TtbButton.TextAndImageCenterChanged(Sender: TObject);
begin
  Paint;
end;

procedure TtbButton.TextFontChanged(Sender: TObject);
var
  f: TFont;
begin
  if ParentFont then
...
procedure TtbButton.TextPosChanged(Sender: TObject);
begin
  Paint;
end;
...
procedure TtbButton.WMHotKey(var Msg: TWMHotKey);
begin
...
procedure TtbButton.CMDialogKey(var Message: TCMDialogKey);
var
  CanClick: Boolean;
begin
  CanClick := false;
...
procedure TtbButton.CMFontChanged(var Message: TMessage);
begin
  inherited;
end;
...
procedure TtbButton.CMParentFontChanged(var Message: TCMParentFontChanged);
var
  f: TFont;
begin
  inherited;
...
function TtbButton.GetParentFont: TFont;
begin
  Result := nil;
  if Parent = nil then
    exit;
...
procedure TtbButton.SetDefault(Value: Boolean);
begin
  FDefault := Value;
  Invalidate;
end;
