...
uses
  ...
  Vcl.ActnList, Vcl.Buttons, Datamodul, fntSqlEdit, fntGrid, Vcl.dbGrids, Vcl.Grids, System.Actions;
...
begin
  ...
  FSqlFileName := GetSqlFileName(FAktualDB) + '.sql';
  FEditor.SaveSqlText(FPath + FSqlFileName);
end;
...
begin
  ...
  begin
    if FAktualDB <> dm.DatabaseName then
    begin
      if FAktualDB > '' then
      begin
        FSqlFileName := GetSqlFileName(FAktualDB) + '.sql';
      ...
      FAktualDB := dm.DatabaseName;
      FSqlFileName := GetSqlFileName(FAktualDB) + '.sql';
      FEditor.LoadSqlText(FPath + FSqlFileName);
      setTablesforHighlighter;
    end;
    FGrid.dbGrid.DataSource := DM.ds_IB;
    Caption := 'SQL-Editor (' + dm.DatabaseName + ')';
  end;
end;
