...
begin
  ...
  SynEdit.Highlighter := FSqlHighlighter;
...
var
  ...
  iEndPos: Integer;
begin
  StartPoint.X := SynEdit.CaretX;
  StartPoint.Y := SynEdit.CaretY-1;
...
begin
  ...
  Result.Y := 0;
  y      := aStartPoint.Y;
  if Y > SynEdit.Lines.Count -1 then
    y := SynEdit.Lines.Count -1;
  for i1 := y downto 0 do
  begin
    s := SynEdit.Lines[i1];
    if i1 = y then
      Delete(s, aStartPoint.X, Length(SynEdit.Lines[i1]));
...
begin
  ...
  if FileExists(FSQlFile) then
    SynEdit.Lines.LoadFromFile(FSqlFile);
end;
...
begin
  ...
  SynEdit.Lines.SaveToFile(aFullFileName);
end;

procedure TfrmSqlEdit.SynEditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Tablename: string;
begin
...
procedure TfrmSqlEdit.SynEditKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  Tablename: string;
begin
...
procedure TfrmSqlEdit.SynEditMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  Tablename: string;
begin
...
begin
  Result := '';
  P.X := SynEdit.CaretX;
  P.Y := SynEdit.CaretY-1;

  LeftPos := P.X;

  if SynEdit.Lines[P.Y] = '' then
    exit;
  if Length(SynEdit.Lines[P.Y]) < P.X then
    exit;


  while LeftPos > 0 do
  begin
    if SynEdit.Lines[P.Y][LeftPos] = ' ' then
      break;
    dec(LeftPos);
  end;
  ...
  while RightPos > 0 do
  begin
    if (SynEdit.Lines[P.Y][RightPos] = ' ') or (SynEdit.Lines[P.Y][RightPos] = '') then
      break;
    inc(RightPos);
  end;

  Result := Trim(copy(SynEdit.Lines[P.Y], LeftPos, RightPos-LeftPos));
...
begin
  ...
  for i1 := y to SynEdit.Lines.Count - 1 do
  begin
    s := SynEdit.Lines[i1];
    if i1 = y then
  ...
  if iPos > 0 then
    ...
    Result.X := Length(SynEdit.Lines[SynEdit.Lines.Count - 1]);
