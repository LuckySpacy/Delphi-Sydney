...
begin //
  ...
  Grd.ColumnHeaders.Add('ISIN');
  Grd.ColumnHeaders.Add('Tickersymbol');
  Grd.ColumnHeaders.Add('EPS (Gewinn pro Aktie)');
  ...
  grd.ColWidths[fCol.ISIN]   := 110;
  grd.ColWidths[fCol.Tickersymbol] := 90;
  grd.ColWidths[fCol.EPS] := 150;
...
begin
  ...
  begin
    ...
    grd.Cells[fCol.ISIN, iRow]  := fAktieList.Item[i1].Isin;
    grd.Cells[fCol.Tickersymbol, iRow]  := fAktieList.Item[i1].Tickersymbol;
    grd.Cells[fCol.EPS, iRow]  := FloatToStr(fAktieList.Item[i1].EPS);
...
begin
  ...
  try
    ...
    if Aktie = nil then
      exit;
    if (fCol.WKN <> ACol) and (fCol.Aktie <> ACol) and (fCol.ISIN <> ACol) and (fCol.Tickersymbol <> ACol) then
      exit;
    ...
    if fCol.ISIN = ACol then
      Aktie.ISIN := grd.Cells[fCol.ISIN, ARow];

    if fCol.TickerSymbol = ACol then
      Aktie.Tickersymbol := grd.Cells[fCol.TickerSymbol, ARow];
