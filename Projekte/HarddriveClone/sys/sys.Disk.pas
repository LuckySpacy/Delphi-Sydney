...
type
  ...
  public
    ...
    function getFilenameWithoutExt(aFullFilename: string): string;
    procedure getDirectories(aPath: string; aStrings: TStrings);
    procedure RemoveAllEmptyDirectories(aPath: String);
  end;
implementation
...
procedure TDisk.getDirectories(aPath: string; aStrings: TStrings);
begin
  GetSubDirectory(aPath, aStrings);
  aStrings.Insert(0, aPath);
end;

procedure TDisk.RemoveAllEmptyDirectories(aPath: String);
var
  List: TStringList;
  i1: Integer;
begin
  ...
  try
    getDirectories(aPath, List);
    for i1 := 0 to List.Count -1 do
      RemoveDir(List.Strings[i1]);
  finally
    FreeAndNil(List);
  end;
end;

function TDisk.getFilenameWithoutExt(aFullFilename: string): string;
var
  Filename: string;
  Ext: string;
begin
