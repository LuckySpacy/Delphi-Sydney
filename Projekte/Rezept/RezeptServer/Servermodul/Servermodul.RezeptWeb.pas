...
var
  WebModuleClass: TComponentClass = TRezeptWeb;
...
uses
  Servermodul.RezeptMethods, Servermodul.RezeptContainer, Web.WebReq;

procedure TRezeptWeb.ServerFunctionInvokerHTMLTag(Sender: TObject; Tag: TTag;
  const TagString: string; TagParams: TStrings; var ReplaceText: string);
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := Servermodul.RezeptMethods.TRezeptServerMethods.ClassName
...
procedure TRezeptWeb.WebModuleDefaultAction(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
begin
...
procedure TRezeptWeb.WebModuleBeforeDispatch(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
begin
...
function TRezeptWeb.AllowServerFunctionInvoker: Boolean;
begin
  Result := (Request.RemoteAddr = '127.0.0.1') or
    (Request.RemoteAddr = '0:0:0:0:0:0:0:1') or (Request.RemoteAddr = '::1');
end;

procedure TRezeptWeb.WebFileDispatcher1BeforeDispatch(Sender: TObject;
  const AFileName: string; Request: TWebRequest; Response: TWebResponse;
  var Handled: Boolean);
var
  D1, D2: TDateTime;
...
procedure TRezeptWeb.WebModuleCreate(Sender: TObject);
begin
