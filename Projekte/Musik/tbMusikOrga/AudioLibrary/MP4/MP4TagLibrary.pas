//********************************************************************************************************************************
//*                                                                                                                              *
//*     MP4 Tag Library 1.0.58.114 © 3delite 2012-2022                                                                           *
...
Const
    MP4TAGLIBRARY_VERSION = $010058114;
    MP4TAGLIBRARY_VERSION_FRIENDLY = '1.0.58.114 (Ivan Llanas branch)';
...
type
    TMP4FREEAtoms = Array of TMP4FREEAtom;

type
    TMP4FREEAtomsList = record
        moovPosition: UInt64;
        FREEAtoms: TMP4FREEAtoms;
    end;
...
type
    ...
    public
        ...
        function Assign(XtraProperty: TMP4XtraProperty): Boolean;
        procedure Clear;
        property Index: Integer read GetIndex;
    end;
    ...
    public
        Name: String;
        Properties: TList<TMP4XtraProperty>;
        Parent: TMP4Tag;
        AlreadyParsed: Boolean;
        Constructor Create;
        Destructor Destroy; override;
        function AddProperty: TMP4XtraProperty;
        function GetAsText(MultipleValues: Boolean): String;
        function GetAsInteger(out Value: Int64): Boolean;
        function GetAsGUID(out Value: TGUID): Boolean;
        function GetAsDateTime(out Value: TDateTime): Boolean;
        function SetAsText(const Value: String; MultipleValues: Boolean): Boolean;
        function SetAsInteger(Value: Int64): Boolean;
        function SetAsGUID(Value: TGUID): Boolean;
        function SetAsDateTime(Value: TDateTime): Boolean;
        function Assign(Xtra: TMP4Xtra): Boolean;
        function GetTotalSize(OnlyValid: Boolean): Cardinal;
        function GetTotalValidCount: Cardinal;
        procedure Clear;
        procedure ReSetAlreadyParsedState;
        property Index: Integer read GetIndex;
    end;
    ...
    public
        ...
        function XtraCount(const FieldName: String; OnlyValid: Boolean): Cardinal;
        //function XtrasTotalSize(const FieldName: String; OnlyValid: Boolean): Cardinal;
        ...
        function GetTextXtra(const FieldName: String; MultipleValues: Boolean): String;
        function SetTextXtra(const FieldName: String; const Value: String; MultipleValues: Boolean): Boolean;
    ...
    function ScanFileForFREEAtoms(FileName: String; var FREEAtomsList: TMP4FREEAtomsList): Integer;
    function RemoveAllRootFREEAtoms(FileName: String; OutputFileName: String): Integer;
    function FileIsMP4(FileName: String; var IsMP4: Boolean): Integer;
...
var
    ...
    MP4TagLibraryParseExceptions: Boolean = False;
...
function SplitString(const Str: String; const Delimiter: Char): TStringArray;
var
    List: TStringList;
    i: Integer;
begin
...
begin
    ...
    try
        ...
        if MP4TagLibraryParseExceptions then begin
            raise;
        end;
    end;
    AlreadyParsed := True;
...
function TMP4Atom.Count: Cardinal;
begin
    Result := Length(Datas);
end;
...
begin
    ...
    try
        ...
        try
            ...
            if ParseMediaAttributes then begin
...
begin
    ...
    while MP4Stream.Position < StartPosition + XtraAtomSize do begin
        ...
        MP4Stream.Read2(PropertyCount, 4);
        PropertyCount := ReverseBytes32(PropertyCount);
        ...
        for i := 0 to PropertyCount - 1 do begin
            ...
            XtraProperty := Xtra.AddProperty;
            XtraProperty.Data.CopyFrom(MP4Stream, PropValueSize);
            XtraProperty.PropType := PropType;
        end;
        Result := True;
    end;
end;
...
begin
    ...
    for i := 0 to Xtras.Count - 1 do begin
        Xtra := Xtras[i];
        if Xtra.GetTotalSize(True) = 0 then begin
            Continue;
        end;
        //* Tag name
        ...
        //* Tag size
        TagSize := 12 + FieldNameSize + Xtra.GetTotalSize(True) + (Xtra.GetTotalValidCount * 6);
        TagSize := ReverseBytes32(TagSize);
        ...
        //* Property count
        PropertyCount := Xtra.GetTotalValidCount;
        PropertyCount := ReverseBytes32(PropertyCount);
        XtraStream.Write(PropertyCount, 4);
        //* Write all the multiple values
        for k := 0 to Xtra.Properties.Count - 1 do begin
            //* Skip invalid tags
            if (Xtra.Properties[k].PropType = 0)
            OR (Xtra.Properties[k].Data.Size = 0)
            then begin
                Continue;
            end;
            //* Add a 2 byte null if not exists that will be written
            NeedToAddANull := False;
            if (Xtra.Properties[k].PropType = MP4_XTRA_BT_UNICODE)
            AND (Xtra.Properties[k].Data.Size > 1)
            then begin
                Xtra.Properties[k].Data.Seek(- 2, soEnd);
                Xtra.Properties[k].Data.Read(WordValue, 2);
                Xtra.Properties[k].Data.Seek(0, soBeginning);
                if WordValue <> 0 then begin
                    NeedToAddANull := True;
                end;
            end;
            //* Tag content size
            PropSize := Xtra.Properties[k].Data.Size + 6;
            if NeedToAddANull then begin
                Inc(PropSize, 2);
            end;
            PropSize := ReverseBytes32(PropSize);
            XtraStream.Write(PropSize, 4);
            //* Tag data type
            PropType := Xtra.Properties[k].PropType;
            PropType := ReverseBytes16(PropType);
            XtraStream.Write(PropType, 2);
            //* The tag conent
            Xtra.Properties[k].Data.Seek(0, soBeginning);
            XtraStream.CopyFrom(Xtra.Properties[k].Data, Xtra.Properties[k].Data.Size);
            if NeedToAddANull then begin
                WordValue := 0;
                XtraStream.Write(WordValue, 2);
            end;
...
begin
    ...
    for i := 0 to Xtras.Count - 1 do begin
        Xtras[i].ReSetAlreadyParsedState;
    end;
end;
...
function TMP4Tag.GetTextXtra(const FieldName: String; MultipleValues: Boolean): String;
var
    Xtra: TMP4Xtra;
begin
    Result := '';
    ...
    if Assigned(Xtra) then begin
        Result := Xtra.GetAsText(MultipleValues);
    end;
end;
...
begin
    ...
    case XtraUsageConversion of
        mp4eucParse, mp4eucParseAndWrite: begin
            ...
            then begin
                XtraContent := GetTextXtra('WM/SharedUserRating', False);
            ...
            if SameText(_name, 'AuthorURL') then begin
                XtraContent := GetTextXtra('WM/AuthorURL', False);
            end;
            if SameText(_name, 'PromotionURL') then begin
                XtraContent := GetTextXtra('WM/PromotionURL', False);
            end;
            if SameText(_name, 'SubTitle') then begin
                XtraContent := GetTextXtra('WM/SubTitle', False);
            end;
            if SameText(_name, 'Category') then begin
                XtraContent := GetTextXtra('WM/Category', True);
            end;
            if SameText(_name, 'Producer') then begin
                XtraContent := GetTextXtra('WM/Producer', False);
            end;
            if SameText(_name, 'EncodedBy') then begin
                XtraContent := GetTextXtra('WM/EncodedBy', False);
            end;
            if SameText(_name, 'Conductor') then begin
                XtraContent := GetTextXtra('WM/Conductor', False);
            end;
            if SameText(_name, 'ParentalRating') then begin
                XtraContent := GetTextXtra('WM/ParentalRating', False);
            end;
            if SameText(_name, 'Writer')
            OR SameText(_name, 'WRITTEN_BY')
            then begin
                XtraContent := GetTextXtra('WM/Writer', False);
            end;
            if SameText(_name, 'Director')
            OR SameText(_name, 'DIRECTED_BY')
            then begin
                XtraContent := GetTextXtra('WM/Director', False);
            end;
            if SameText(_name, 'Publisher') then begin
                XtraContent := GetTextXtra('WM/Publisher', False);
            end;
            if SameText(_name, 'Mood') then begin
                XtraContent := GetTextXtra('WM/Mood', False);
            end;
            if SameText(_name, 'ContentDistributor')
            OR SameText(_name, 'DISTRIBUTED_BY')
            then begin
                XtraContent := GetTextXtra('WM/ContentDistributor', False);
            end;
            if SameText(_name, 'InitialKey')
            OR SameText(_name, 'KEY')
            then begin
                XtraContent := GetTextXtra('WM/InitialKey', False);
            end;
            if SameText(_name, 'Period') then begin
                XtraContent := GetTextXtra('WM/Period', False);
            end;
            if SameText(_name, 'PartOfSet') then begin
                XtraContent := GetTextXtra('WM/PartOfSet', False);
            end;
            //* ID GUIDs
            if SameText(_name, 'MediaClassPrimaryID') then begin
                XtraContent := GetTextXtra('WM/MediaClassPrimaryID', False);
            end;
            if SameText(_name, 'MediaClassSecondaryID') then begin
                XtraContent := GetTextXtra('WM/MediaClassSecondaryID', False);
            end;
            if SameText(_name, 'WMContentID') then begin
                XtraContent := GetTextXtra('WM/WMContentID', False);
            end;
            if SameText(_name, 'WMCollectionID') then begin
                XtraContent := GetTextXtra('WM/WMCollectionID', False);
            end;
            if SameText(_name, 'WMCollectionGroupID') then begin
                XtraContent := GetTextXtra('WM/WMCollectionGroupID', False);
            end;
        end;
    end;
...
function TMP4Tag.SetTextXtra(const FieldName, Value: String; MultipleValues: Boolean): Boolean;
var
    Xtra: TMP4Xtra;
    Index: Integer;
begin
    ...
    if Value <> '' then begin
        ...
        if Assigned(Xtra) then begin
            Result := Xtra.SetAsText(Value, MultipleValues);
        end;
    end else begin
        Index := FindAtomXtraIndex(FieldName);
        if Index > - 1 then begin
    ...
    then begin
        ...
        case XtraUsageConversion of
            mp4eucWrite, mp4eucParseAndWrite: begin
                ...
                if SameText(_name, 'AuthorURL') then begin
                    SetXtra := SetTextXtra('WM/AuthorURL', Value, False);
                end;
                if SameText(_name, 'PromotionURL') then begin
                    SetXtra := SetTextXtra('WM/PromotionURL', Value, False);
                end;
                if SameText(_name, 'SubTitle') then begin
                    SetXtra := SetTextXtra('WM/SubTitle', Value, False);
                end;
                if SameText(_name, 'Category') then begin
                    SetXtra := SetTextXtra('WM/Category', Value, True);
                end;
                if SameText(_name, 'Producer') then begin
                    SetXtra := SetTextXtra('WM/Producer', Value, False);
                end;
                if SameText(_name, 'EncodedBy') then begin
                    SetXtra := SetTextXtra('WM/EncodedBy', Value, False);
                end;
                if SameText(_name, 'Conductor') then begin
                    SetXtra := SetTextXtra('WM/Conductor', Value, False);
                end;
                if SameText(_name, 'ParentalRating') then begin
                    SetXtra := SetTextXtra('WM/ParentalRating', Value, False);
                end;
                if SameText(_name, 'Writer')
                OR SameText(_name, 'WRITTEN_BY')
                then begin
                    SetXtra := SetTextXtra('WM/Writer', Value, False);
                end;
                if SameText(_name, 'Director')
                OR SameText(_name, 'DIRECTED_BY')
                then begin
                    SetXtra := SetTextXtra('WM/Director', Value, False);
                end;
                if SameText(_name, 'Publisher') then begin
                    SetXtra := SetTextXtra('WM/Publisher', Value, False);
                end;
                if SameText(_name, 'Mood') then begin
                    SetXtra := SetTextXtra('WM/Mood', Value, False);
                end;
                if SameText(_name, 'ContentDistributor')
                OR SameText(_name, 'DISTRIBUTED_BY')
                then begin
                    SetXtra := SetTextXtra('WM/ContentDistributor', Value, False);
                end;
                if SameText(_name, 'InitialKey')
                OR SameText(_name, 'KEY')
                then begin
                    SetXtra := SetTextXtra('WM/InitialKey', Value, False);
                end;
                if SameText(_name, 'Period') then begin
                    SetXtra := SetTextXtra('WM/Period', Value, False);
                end;
                if SameText(_name, 'PartOfSet') then begin
                    SetXtra := SetTextXtra('WM/PartOfSet', Value, False);
                end;
                //* ID GUIDs
                if SameText(_name, 'MediaClassPrimaryID') then begin
...
function TMP4Tag.XtraCount(const FieldName: String; OnlyValid: Boolean): Cardinal;
var
    i: Integer;
begin
    Result := 0;
    for i := 0 to Xtras.Count - 1 do begin
        if Xtras[i].Name = FieldName then begin
            //* Skip invalid tags
            if OnlyValid then begin
                if (Xtras[i].Name = '')
                OR (Xtras[i].GetTotalSize(OnlyValid) = 0)
                then begin
                    Continue;
                end;
            end;
...
(*
function TMP4Tag.XtrasTotalSize(const FieldName: String; OnlyValid: Boolean): Cardinal;
var
    i: Integer;
    WordValue: Word;
begin
    ...
    for i := 0 to Xtras.Count - 1 do begin
        if Xtras[i].Name = FieldName then begin
            ...
            if OnlyValid then begin
                if (Xtras[i].Name = '')
                OR (Xtras[i].PropType = 0)
                OR (Xtras[i].Data.Size = 0)
                then begin
                    Continue;
                end;
            end;
            Inc(Result, Xtras[i].Data.Size);
            //* Add a 2 byte null if not exists that will be written
            if (Xtras[i].PropType = MP4_XTRA_BT_UNICODE)
            AND (Xtras[i].Data.Size > 1)
            then begin
                Xtras[i].Data.Seek(- 2, soEnd);
                Xtras[i].Data.Read(WordValue, 2);
                Xtras[i].Data.Seek(0, soBeginning);
                if WordValue <> 0 then begin
                    Inc(Result, 2);
                end;
            end;
...
function TMP4Xtra.AddProperty: TMP4XtraProperty;
begin
    Result := TMP4XtraProperty.Create;
    Result.Parent := Self;
...
begin
    if Assigned(Xtra) then begin
        Self.Clear;
        Self.Name := Xtra.Name;
        for i := 0 to Xtra.Properties.Count - 1 do begin
            XtraProperty := AddProperty;
            XtraProperty.Assign(Xtra.Properties[i]);
        end;
    end else begin
        Clear;
    end;
...
var
    i: Integer;
begin
    for i := Properties.Count - 1 downto 0 do begin
        Properties[i].Free;
    end;
    Properties.Clear;
    //Name := '';
...
begin
    inherited;
    //Data := TMemoryStream.Create;
    Properties := TList<TMP4XtraProperty>.Create;
end;
...
begin
    Result := False;
    Value := TGUID.Create('{00000000-0000-0000-0000-000000000000}');
...
begin
    ...
    Properties[0].GetAsInteger(Value);
    Result := True;
    AlreadyParsed := True;
end;
...
begin
    ...
    Properties[0].GetAsDateTime(Value);
    Result := True;
    AlreadyParsed := True;
end;

function TMP4Xtra.GetAsText(MultipleValues: Boolean): String;
var
    i: Integer;
begin
    Result := '';
    ...
    if Properties.Count = 1 then begin
        Result := Properties[0].GetAsText;
        Exit;
    end;
    if MultipleValues then begin
        for i := 0 to Properties.Count - 1 do begin
            if i = Properties.Count - 1 then begin
                Result := Result + Properties[i].GetAsText;
            end else begin
                Result := Result + Properties[i].GetAsText + ';';
            end;
        end;
...
function TMP4Xtra.GetTotalSize(OnlyValid: Boolean): Cardinal;
var
    i: Integer;
    WordValue: Word;
begin
    ...
    for i := 0 to Properties.Count - 1 do begin
        //* Skip invalid tags
        if OnlyValid then begin
            if (Properties[i].PropType = 0)
            OR (Properties[i].Data.Size = 0)
            then begin
                Continue;
            end;
        end;
        Inc(Result, Properties[i].Data.Size);
        //* Add a 2 byte null if not exists that will be written
        if (Properties[i].PropType = MP4_XTRA_BT_UNICODE)
        AND (Properties[i].Data.Size > 1)
        then begin
            Properties[i].Data.Seek(- 2, soEnd);
            Properties[i].Data.Read(WordValue, 2);
            Properties[i].Data.Seek(0, soBeginning);
            if WordValue <> 0 then begin
                Inc(Result, 2);
            end;
        end;
...
function TMP4Xtra.GetTotalValidCount: Cardinal;
var
    i: Integer;
begin
    Result := 0;
...
procedure TMP4Xtra.ReSetAlreadyParsedState;
var
    i: Integer;
begin
    AlreadyParsed := False;
    for i := 0 to Properties.Count - 1 do begin
        Properties[i].AlreadyParsed := False;
    end;
end;
...
begin
    ...
    Properties[0].SetAsInteger(Value);
    Result := True;
end;

function TMP4Xtra.SetAsText(const Value: String; MultipleValues: Boolean): Boolean;
var
    {$IFDEF FPC}
    Values: TStringArray;
    {$ELSE}
...
begin
    ...
    if MultipleValues then begin
        ...
        for i := 0 to High(Values) do begin
            XtraProperty := AddProperty;
            XtraProperty.SetAsText(Values[i]);
        end;
    end else begin
        if Properties.Count = 0 then begin
            AddProperty;
        end;
        Properties[0].SetAsText(Value);
    end;
    Result := True;
end;
...
begin
    ...
    Properties[0].SetAsDateTime(Value);
    Result := True;
end;
...
function TMP4XtraProperty.Assign(XtraProperty: TMP4XtraProperty): Boolean;
begin
    if Assigned(XtraProperty) then begin
        Self.Clear;
        Self.PropType := XtraProperty.PropType;
        Self.Data.Clear;
        Self.Data.CopyFrom(XtraProperty.Data, 0);
        Self.Data.Seek(0, soBeginning);
        XtraProperty.Data.Seek(0, soBeginning);
    end else begin
        Clear;
    end;
    Result := True;
...
procedure TMP4XtraProperty.Clear;
begin
    PropType := 0;
    Data.Clear;
end;

constructor TMP4XtraProperty.Create;
begin
    inherited;
    Data := TMemoryStream.Create;
end;

destructor TMP4XtraProperty.Destroy;
begin
    Clear;
    FreeAndNil(Data);
    inherited;
...
function TMP4XtraProperty.GetAsDateTime(out Value: TDateTime): Boolean;
var
    Data64: UInt64;
begin
    Value := 0;
...
function TMP4XtraProperty.GetAsGUID(out Value: TGUID): Boolean;
begin
    Data.Seek(0, soBeginning);
...
function TMP4XtraProperty.GetAsInteger(out Value: Int64): Boolean;
var
    DataByte: Byte;
    DataWord: Word;
    DataDWord: DWord;
...
function TMP4XtraProperty.GetAsText: String;
var
...
begin
    ...
    if PropType = MP4_XTRA_BT_GUID then begin
        ...
        {$IFDEF NEXTGEN}
        if Result[Length(Result) - 1] = #0 then begin
...
function TMP4XtraProperty.GetIndex: Integer;
begin
    Result := Self.Parent.Properties.IndexOf(Self);
end;

function TMP4XtraProperty.SetAsDateTime(Value: TDateTime): Boolean;
...
function TMP4XtraProperty.SetAsGUID(Value: TGUID): Boolean;
begin
    Data.Clear;
...
function TMP4XtraProperty.SetAsInteger(Value: Int64): Boolean;
begin
    Data.Clear;
    Data.Write(Value, 8);
    Data.Seek(0, soBeginning);
...
function TMP4XtraProperty.SetAsText(const Value: String): Boolean;
var
    DataBytes: TBytes;
begin
    Data.Clear;
...
var
    ...
    FREEAtomSize: Int64;
    Is64BitAtomSize: Boolean;
    moovAtomFound: Boolean;
begin
    FREEAtomsList.moovPosition := 0;
    SetLength(FREEAtomsList.FREEAtoms, 0);
    try
    ...
    try
        try
            //Result := MP4TAGLIBRARY_ERROR_NO_TAG_FOUND;
            try
                ReadAtomHeader(FileStream, AtomName, AtomSize, Is64BitAtomSize, False);
            except
                //* Will except if not an MP4 file and MP4TagLibraryFailOnCorruptFile is True
            end;
            if NOT IsSameAtomName(AtomName, 'ftyp') then begin
            ...
            // Continue gathering the atoms for the file metadata.
            FileStream.Seek(AtomSize - 8, soCurrent);
            repeat
                ReadAtomHeader(FileStream, AtomName, moovAtomSize, Is64BitAtomSize);
                if IsSameAtomName(AtomName, 'moov') then begin
                    FREEAtomsList.moovPosition := FileStream.Position;
                    repeat
                        ReadAtomHeader(FileStream, AtomName, AtomSize, Is64BitAtomSize);
                        if IsSameAtomName(AtomName, 'udta') then begin
                            repeat
                                ReadAtomHeader(FileStream, AtomName, AtomSize, Is64BitAtomSize);
                                if IsSameAtomName(AtomName, 'meta') then begin
                                    FileStream.Seek(4, soCurrent);
                                    repeat
                                        ReadAtomHeader(FileStream, AtomName, FREEAtomSize, Is64BitAtomSize);
                                        if IsSameAtomName(AtomName, 'free') then begin
                                            SetLength(FREEAtomsList.FREEAtoms, Length(FREEAtomsList.FREEAtoms) + 1);
                                            FREEAtomsList.FREEAtoms[Length(FREEAtomsList.FREEAtoms) - 1].AtomSize := FREEAtomSize;
                                            FREEAtomsList.FREEAtoms[Length(FREEAtomsList.FREEAtoms) - 1].AtomPosition := FileStream.Position;
                                        end;
                                        if Is64BitAtomSize then begin
                                            FileStream.Seek(FREEAtomSize - 16, soCurrent);
                                        end else begin
                                            FileStream.Seek(FREEAtomSize - 8, soCurrent);
                                        end;
                                    until (FileStream.Position >= FileStream.Size)
                                    OR (FileStream.Position + FREEAtomSize >= FileStream.Size);
                                end else if IsSameAtomName(AtomName, 'free') then begin
                                    SetLength(FREEAtomsList.FREEAtoms, Length(FREEAtomsList.FREEAtoms) + 1);
                                    FREEAtomsList.FREEAtoms[Length(FREEAtomsList.FREEAtoms) - 1].AtomSize := AtomSize;
                                    FREEAtomsList.FREEAtoms[Length(FREEAtomsList.FREEAtoms) - 1].AtomPosition := FileStream.Position;
                                end;
                                if Is64BitAtomSize then begin
                                    FileStream.Seek(AtomSize - 16, soCurrent);
                                end else begin
                                    FileStream.Seek(AtomSize - 8, soCurrent);
                                end;
                            until (FileStream.Position >= FileStream.Size);
                    ...
                    SetLength(FREEAtomsList.FREEAtoms, Length(FREEAtomsList.FREEAtoms) + 1);
                    FREEAtomsList.FREEAtoms[Length(FREEAtomsList.FREEAtoms) - 1].AtomSize := moovAtomSize;
                    FREEAtomsList.FREEAtoms[Length(FREEAtomsList.FREEAtoms) - 1].AtomPosition := FileStream.Position;
                end;
                if Is64BitAtomSize then begin
                    FileStream.Seek(moovAtomSize - 16, soCurrent);
                end else begin
                    FileStream.Seek(moovAtomSize - 8, soCurrent);
                end;
            until (FileStream.Position >= FileStream.Size)
            OR (moovAtomSize = 0);
            Result := MP4TAGLIBRARY_SUCCESS;
...
begin
    try
        InputFileStream := TFileStream.Create(FileName, fmOpenRead OR fmShareDenyWrite);
    except
        Result := MP4TAGLIBRARY_ERROR_OPENING_FILE;
        Exit;
    end;
    ...
    try
        try
            try
                ReadAtomHeader(InputFileStream, AtomName, AtomSize, Is64BitAtomSize, False);
            except
                //* Will except if not an MP4 file and MP4TagLibraryFailOnCorruptFile is True
            end;
            if NOT IsSameAtomName(AtomName, 'ftyp') then begin
            ...
            mdatPreviousLocation := MP4mdatAtomLocation(InputFileStream);
            InputFileStream.Seek(0, soBeginning);
            repeat
                ReadAtomHeader(InputFileStream, AtomName, AtomSize, Is64BitAtomSize);
                if NOT IsSameAtomName(AtomName, 'free') then begin
                    if Is64BitAtomSize then begin
                        InputFileStream.Seek(- 16, soCurrent);
                    end else begin
                        InputFileStream.Seek(- 8, soCurrent);
                    end;
                    OutputFileStream.copyFrom(InputFileStream, AtomSize);
                end else begin
                    if Is64BitAtomSize then begin
                        InputFileStream.Seek(AtomSize - 16, soCurrent);
                    end else begin
                        InputFileStream.Seek(AtomSize - 8, soCurrent);
                    end;
                end;
            until (InputFileStream.Position >= InputFileStream.Size)
            OR (AtomSize = 0);
            //* Check and update stco/co64 atom
            OutputFileStream.Seek(0, soBeginning);
            mdatNewLocation := MP4mdatAtomLocation(OutputFileStream);
            OutputFileStream.Seek(0, soBeginning);
            if mdatNewLocation - mdatPreviousLocation <> 0 then begin
                OutputFileStream.Seek(0, soBeginning);
                if NOT MP4UpdatestcoAtom(OutputFileStream, mdatNewLocation - mdatPreviousLocation) then begin
                    Result := MP4TAGLIBRARY_ERROR_UPDATE_stco;
                    Exit;
                end;
                OutputFileStream.Seek(0, soBeginning);
                if NOT MP4Updateco64Atom(OutputFileStream, mdatNewLocation - mdatPreviousLocation) then begin
                    Result := MP4TAGLIBRARY_ERROR_UPDATE_co64;
                    Exit;
                end;
            end;
