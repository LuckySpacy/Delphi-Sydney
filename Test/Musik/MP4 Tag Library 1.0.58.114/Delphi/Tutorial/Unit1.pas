//********************************************************************************************************************************
//*                                                                                                                              *
//*     MP4 Tag Library Tutorial © 3delite 2012-2022                                                                             *
...
begin
    ...
    //MP4Tag.ParseXtra := True;
    //MP4Tag.ParseCoverArts := False; //* This speeds up loading if cover arts are not needed (like scanning a folder), DO NOT SAVE THE TAGS WHEN ParseCoverArts WAS FALSE WHEN LOADING!!!
    ...
    if (MP4Tag.VideoWidth <> 0)
    AND (MP4Tag.VideoHeight <> 0)
    then begin
        Memo1.Lines.Add('Video ' + IntToStr(MP4Tag.VideoWidth) + ' x ' + IntToStr(MP4Tag.VideoHeight));
    end;
    //* Audio track attributes
    case MP4Tag.AudioFormat of
        mp4afUnknown: Memo1.Lines.Add('Unknown audio track format');
        mp4afAAC: Memo1.Lines.Add('AAC audio, ' + IntToStr(MP4Tag.AudioSampleRate) + ' Hz, ' + IntToStr(MP4Tag.AudioResolution) + ' bit, ' + IntToStr(MP4Tag.AudioChannelCount) + ' channels');
        mp4afALAC: Memo1.Lines.Add('ALAC audio, ' + IntToStr(MP4Tag.AudioSampleRate) + ' Hz, ' + IntToStr(MP4Tag.AudioResolution) + ' bit, ' + IntToStr(MP4Tag.AudioChannelCount) + ' channels');
        mpfafAC3: Memo1.Lines.Add('AC3 audio, ' + IntToStr(MP4Tag.AudioSampleRate) + ' Hz, ' + IntToStr(MP4Tag.AudioResolution) + ' bit, ' + IntToStr(MP4Tag.AudioChannelCount) + ' channels');
    end;
    ...
    for i := 0 to MP4Tag.Count - 1 do begin
        ...
        if IsSameAtomName(MP4Tag.Atoms[i].ID, 'covr') then begin
            if MP4Tag.Atoms[i].Datas[0].Data.Size > 0 then begin
