...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, UITypes, ID3v1Library,
...
begin
    ...
    //ID3v2Tag.ParseCoverArts := False; //* This speeds up loading if cover arts are not needed (like scanning a folder), DO NOT SAVE THE TAGS WHEN ParseCoverArts WAS FALSE WHEN LOADING!!!

    //ID3v2Tag.MPEGSearchLength := 1024 * 100;
    ...
    //* Playtime
    LabelPlayTime.Caption := mSec2Time(Trunc(ID3v2Tag.PlayTime * 1000));

    //* Bit rate
    if ID3v2Tag.MPEGInfo.VBR then begin
        LabelBitRate.Caption := 'VBR ' + IntToStr(ID3v2Tag.BitRate) + ' kbps';
    end else begin
        LabelBitRate.Caption := IntToStr(ID3v2Tag.BitRate) + ' kbps';
    end;

    case ID3v2Tag.MPEGInfo.ChannelMode of
       tmpegcmUnknown: LabelChannels.Caption := 'Unknown';
       tmpegcmMono: LabelChannels.Caption := 'Mono';
       tmpegcmDualChannel: LabelChannels.Caption := 'Dual channel';
       tmpegcmJointStereo: LabelChannels.Caption := 'Joint stereo';
       tmpegcmStereo: LabelChannels.Caption := 'Stereo';
    end;

    //* The ID3v2 tag was unsynchronised
    CheckBox1.Checked := ID3v2Tag.WasUnsynchronised;
    ...
    //* Get Genre
    ComboBox2.Text := ID3v2DecodeGenre(ID3v2Tag.GetUnicodeText('TCON'));
    ...
    //* Example to parse the 'TXXX' frames
    {
...
begin
    ...
    //* Save the ID3v2 Tag
    ErrorCode := ID3v2Tag.SaveToFile(Edit1.Text{$IFDEF DEBUG}, False{$ENDIF});
